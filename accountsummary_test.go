...
import (
	...
	generator_mocks "github.com/cshep4/crypto-dot-com-exchange-go/internal/mocks/generator"
)
...
func TestClient_GetAccountSummary_Error(t *testing.T) {
		...
		},
		{
			...
			client: http.Client{
				Transport: roundTripper{
					...
					response: cdcexchange.AccountSummaryResponse{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var (
				idGenerator = generator_mocks.NewMockIDGenerator(ctrl)
				now         = time.Now()
	...
	tests := []struct {
	...
	}{
		{
			...
			handlerFunc: func(w http.ResponseWriter, r *http.Request) {
				...
				var body cdcexchange.APIRequest
